...
static size_t alloc_untrace(void *ptr);
...
static void alloc_trace_new(void *ptr, size_t size) {
	...
	if (a != NULL) {
		...
		if (alloc_batch == NULL || alloc_batch->used == ALLOCATION_BATCH_SIZE) {
			alloc_batch = mmap(NULL, sizeof(*alloc_batch),
...
static size_t alloc_untrace(void *ptr) {
	if (alloc_is_static(ptr)) return 0;
	if (depth > 1) return 0;
	heaph_assert(is_init_done);
	if (is_exit_done) return 0;
	spinlock_acq(&allocs_lock);
...
static const struct allocation *alloc_find(void *ptr) {
	if (alloc_is_static(ptr)) return NULL;
	if (depth > 1) return NULL;
	heaph_assert(is_init_done);
	if (is_exit_done) return NULL;
	spinlock_acq(&allocs_lock);
	struct allocation *a = allocs;
	while (a != NULL) {
		if (a->mem == ptr) {
...
static bool trace_sym_file_starts_with(const struct symbol *sym, const char *file) {
...
static bool trace_sym_is_internal(const struct symbol *sym) {
	...
	if (trace_sym_is_func(sym, "_IO_doallocbuf")) return true;
...
static void heaph_atexit(void) {
	...
	heaph_printf("HH: found %lld leaks (%llu bytes)\n",
		(long long)count, (long long)leak_size);
...
static void heaph_init(void) {
	...
	int (*sym_getaddrinfo)(const char *, const char *,
...
void *realloc(void *ptr, size_t size) {
	...
	if (content_mode == CONTENT_MODE_TRASH && res != NULL) {
		if (ptr == NULL) {
			...
			memset((char *)res + alloc->size, '#', size - alloc->size);
		}
	}
