...
struct chat_server {
    ...
    struct peer *peers;
    struct qn *hq, *tq;
    char *sin; size_t sin_cap, sin_len;
};

static void chat_server_clear_queue(struct chat_server *s){
    struct chat_message *m;
    while ((m = chat_server_pop_next(s))) chat_message_delete(m);
}
...
static void message_store(struct chat_server *s, struct peer *f,
...
static void broadcast(struct chat_server *s, struct peer *f,
                      const char *d, size_t l){
    char *wire; size_t wl;
#if NEED_AUTHOR
    ...
    wl = al + 1 + l + 1;
    wire = malloc(wl);
    memcpy(wire, a, al); wire[al] = ' ';
    memcpy(wire + al + 1, d, l); wire[wl-1] = '\n';
#else
    wl = l + 1;
    wire = malloc(wl);
    memcpy(wire, d, l); wire[wl-1] = '\n';
#endif
    for (struct peer *p = s->peers; p; p = p->next){
        if (p != f) peer_enqueue(p, wire, wl);
    }
    free(wire);
}

static void peer_got_line(struct chat_server *s, struct peer *p,
...
#endif
    size_t clen; char *c = str_trim(b, l, &clen);
    if (clen){
...
static void peer_consume_in(struct chat_server *s, struct peer *p){
    ...
    if (rem && b != p->in) memmove(p->in, b, rem);
    p->ilen = rem;
}

static void peer_write(struct peer *p){
    while (p->opos < p->olen){
        ssize_t n = send(p->sock, p->out + p->opos,
                         p->olen - p->opos, MSG_NOSIGNAL);
        if (n > 0) p->opos += n;
        else if (errno == EAGAIN || errno == EWOULDBLOCK) break;
        else { p->opos = p->olen = 0; break; }
    }
...
int chat_server_update(struct chat_server *s, double timeout){
    ...
    size_t np = 0;
    for (struct peer *p = s->peers; p; p = p->next) ++np;
    size_t nf = np + 1;
    struct pollfd *pf = alloca(sizeof *pf * nf);
    memset(pf, 0, sizeof *pf * nf);
    pf[0].fd = s->lsock; pf[0].events = POLLIN;
    size_t idx = 1;
    ...
    int rc = poll(pf, nf, ms);
    if (rc < 0) return CHAT_ERR_SYS;
    bool prog = false;
    struct peer **pp = &s->peers;
    for (idx = 0; idx < np && *pp; ){
        struct peer *p = *pp;
        struct pollfd *f = &pf[1 + idx];
        if (f->revents & (POLLERR | POLLHUP)){
            *pp = p->next; peer_del(p); prog = true; continue;
        }
        if (f->revents & POLLIN){
            if (p->icap - p->ilen < 4096){
                size_t c = p->icap ? p->icap * 2 : 4096;
                p->in = xrealloc(p->in, c); p->icap = c;
            }
            ssize_t n = recv(p->sock, p->in + p->ilen,
                             p->icap - p->ilen, 0);
            if (n > 0){ p->ilen += n; peer_consume_in(s, p); prog = true; }
            else if (n == 0 || (n < 0 && errno != EAGAIN && errno != EWOULDBLOCK)){
                *pp = p->next; peer_del(p); prog = true; continue;
            }
        }
        if ((f->revents & POLLOUT) && p->olen > p->opos){
            peer_write(p); prog = true;
        }
        pp = &p->next; ++idx;
    }
    if (pf[0].revents & POLLIN){
        for (;;){
            ...
            set_nonblock(cs);
            struct peer *np2 = peer_new(cs);
            np2->next = s->peers; s->peers = np2;
            prog = true;
        }
    }
...
int chat_server_feed(struct chat_server *s, const char *buf, uint32_t sz){
    ...
    while (pos < s->sin_len){
        ...
        if (l && s->sin[pos + l - 1] == '\r') --l;
        size_t clen; char *c = str_trim(s->sin + pos, l, &clen);
        if (clen) broadcast(s, NULL, c, clen);
        free(c);
        pos = (nl - s->sin) + 1;
    }
    if (pos){
