...
struct mnode { struct chat_message *m; struct mnode *n; };

static void mq_push(struct mnode **h, struct mnode **t, struct chat_message *m){
...
void chat_client_delete(struct chat_client *c){
    ...
    free(c->in); free(c->out); free(c->pend);
#if NEED_AUTHOR
    free(c->name);
#endif
    struct chat_message *m;
    while((m=mq_pop(&c->hq,&c->tq))) chat_message_delete(m);
    free(c);
}

static int split_addr(const char *addr, char **h, char **p){
    const char *c=strrchr(addr,':');
    if(!c||c==addr||c[1]=='\0') return CHAT_ERR_NO_ADDR;
...
int chat_client_connect(struct chat_client *c,const char *addr){
    ...
    if(getaddrinfo(host,port,&hints,&ai)!=0){free(host);free(port);return CHAT_ERR_NO_ADDR;}
    free(host); free(port);
    int s=-1;
    for(struct addrinfo *p=ai;p;p=p->ai_next){
        ...
        if(s<0) continue;
        set_nonblock(s);
        if(connect(s,p->ai_addr,p->ai_addrlen)==0||errno==EINPROGRESS) break;
        close(s); s=-1;
    }
    freeaddrinfo(ai);
...
static void produce_msg(struct chat_client *c,const char *b,size_t l){
    ...
    if(sp){
        ...
        const char *bd=sp+1;
        while(bd<b+l&&isspace((unsigned char)*bd))++bd;
...
int chat_client_feed(struct chat_client *c,const char *buf,uint32_t sz){
    ...
    while(true){
        ...
        if(tl){ out_append(c,t,tl); out_append(c,"\n",1); }
        free(t);
        p=nl+1;
    }
    size_t r=c->pend+c->pend_len-p;
    if(r&&p!=c->pend) memmove(c->pend,p,r);
    c->pend_len=r;
    return 0;
}
...
int chat_client_update(struct chat_client *c,double timeout){
    ...
    while(c->out_pos<c->out_len){
        ...
        if(n>0){c->out_pos+=n;prog=true;if(c->out_pos==c->out_len)c->out_pos=c->out_len=0;}
        else if(errno!=EAGAIN&&errno!=EWOULDBLOCK) return CHAT_ERR_SYS;
        else break;
    }
    for(;;){
        ...
        if(n>0){c->in_len+=n;consume_in(c);prog=true;}
        else if(n==0) return CHAT_ERR_SYS;
        else if(errno!=EAGAIN&&errno!=EWOULDBLOCK) return CHAT_ERR_SYS;
        else break;
    }
    if(prog||c->out_pos<c->out_len) return 0;
    struct pollfd pfd={c->sock, (c->out_pos<c->out_len?POLLOUT:0)|POLLIN,0};
    ...
    if((pfd.revents&POLLOUT)&&c->out_pos<c->out_len){
        ssize_t n=send(c->sock,c->out+c->out_pos,c->out_len-c->out_pos,MSG_NOSIGNAL);
        if(n>0){c->out_pos+=n;if(c->out_pos==c->out_len)c->out_pos=c->out_len=0;prog=true;}
        else if(errno!=EAGAIN&&errno!=EWOULDBLOCK) return CHAT_ERR_SYS;
    }
    if(pfd.revents&POLLIN){
        if(c->in_cap-c->in_len<4096){
            c->in_cap*=2; c->in=xrealloc(c->in,c->in_cap);
        }
        ssize_t n=recv(c->sock,c->in+c->in_len,c->in_cap-c->in_len,0);
