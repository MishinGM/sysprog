...
static void test_io(void) {
    ...
    fd1 = ufs_open("file", 0);
    unit_fail_if(fd1 == -1);
    unit_check(ufs_write(fd1, "1\0" "2\0" "3\0", 6) == 6, "data with zeros");
    ufs_close(fd1);
    fd1 = ufs_open("file", 0);
    unit_fail_if(fd1 == -1);
    unit_check(ufs_read(fd1, buffer, 6) == 6, "read data with zeros");
    unit_check(memcmp(buffer, "1\0" "2\0" "3\0", 6) == 0, "check with zeros");
    ufs_close(fd1);
...
static void test_delete(void) {
    ...
    ufs_close(tmp);
    unit_check(ufs_write(fd2, "a", 1) == 1, "write into an fd opened before deletion");
    unit_check(ufs_read(fd3, &c1, 1) == 1, "read from another opened fd - it sees the data");
    unit_check(c1 == 'a', "exactly the same data");
    unit_check(ufs_write(fd3, "bc", 2) == 2, "write into it and the just read data is not overwritten");
    ...
    unit_check(fd4 == -1, "the existing 'ghost' file is not visible anymore for new opens");
    unit_check(ufs_errno() == UFS_ERR_NO_FILE, "errno is set");
...
static void test_max_file_size(void) {
    ...
    unit_check(ufs_write(fd1, buf, 1) == 1, "write inside the file using another descriptor");
    ufs_close(fd1);
    unit_fail_if(ufs_close(fd) != 0);
    fd = ufs_open("file", 0);
    unit_fail_if(fd == -1);
...
static void test_rights(void) {
    ...
    unit_check(memcmp(buf1, buf2, buf1_size) == 0, "and data was not overwritten");
    unit_fail_if(ufs_close(fd) != 0);
...
static void test_resize(void) {
    ...
    unit_check(rc == sizeof(buffer), "opened descriptor beyond new border still works");
    unit_fail_if(ufs_close(fd) != 0);
    unit_fail_if(ufs_delete("file") != 0);
    fd = ufs_open("file", UFS_CREATE);
    unit_fail_if(fd == -1);
    ...
    unit_fail_if(rc != 8);
    unit_check(memcmp(buffer, "aaaafter", 8) == 0, "descriptor was affected by resize and still works");
    unit_fail_if(ufs_close(fd2) != 0);
    unit_fail_if(ufs_close(fd) != 0);
    unit_fail_if(ufs_delete("file") != 0);
    unit_test_finish();
