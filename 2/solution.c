...
#include <windows.h>
#include <process.h>
#define waitpid(pid, status, options) _cwait(status, pid, 0)
#else
#include <sys/wait.h>
#endif
#include <fcntl.h>
...
static int xp(struct expr **cmds, int n, char *of, int ot, int bg) {
    if(n == 1) {
        ...
        if(!strcmp(c, "exit") && !of) {
            int ec = 0;
            if(cmds[0]->cmd.arg_count > 0)
                ec = atoi(cmds[0]->cmd.args[0]);
            exit(ec);
        }
    }
    int i, ret = 0;
    ...
    for(i = 0; i < n; i++) {
        ...
        if(pid == 0) {
            ...
            if(!strcmp(cmds[i]->cmd.exe, "exit")) {
                int ec = 0;
                if(cmds[i]->cmd.arg_count > 0)
                    ec = atoi(cmds[i]->cmd.args[0]);
                _exit(ec);
            }
            execvp(cmds[i]->cmd.exe, (char* const*)((char*[]){cmds[i]->cmd.exe, cmds[i]->cmd.arg_count > 0 ? cmds[i]->cmd.args[0] : NULL, NULL}));
            execvp(cmds[i]->cmd.exe, cmds[i]->cmd.args);
...
static int exec_line(struct command_line *cl) {
    ...
    while(e) {
        ...
        ret = xp(arr, cnt, cl->out_file, (cl->out_type == OUTPUT_TYPE_FILE_NEW) ? ONEW : APP, cl->is_background);
        free(arr);
...
int main(void) {
    ...
    while((r = read(STDIN_FILENO, buf, sizeof(buf))) > 0) {
        ...
        while(1) {
            ...
            if(err == PARSER_ERR_NONE && cl == NULL)
                break;
